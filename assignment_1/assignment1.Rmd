...
impute.to.mean <- function(x) {
    if (is.double(x)){x[is.na(x)] <- mean(x, na.rm=TRUE)} else {x[is.na(x)] <- mode(x)}
    return(x)
  }
```
...
numcols <- cohort.dt %>% select(bp) %>% colnames
cohort.dt %>% .[, (numcols) := lapply(.SD, impute.to.mean),.SDcols = numcols]
