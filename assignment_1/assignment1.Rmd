...
#' the NAs with its mean / mode if tha vector is numeric or categorical respectively.
...
impute.to.mean <- function(x) {
    if (is.double(x)){
      ...
      uniquex = unique(x)
      x[is.na(x)] <- uniquex[which.max(tabulate(match(x,uniquex)))]
    } else {
      uniquex = unique(x)
#      x[is.na(x)] <- which.max(unique(x), na.rm=TRUE)
      x[is.na(x)] <- uniquex[which.max(tabulate(match(x,uniquex)))]
...
# bp,diabetes,urea,creatinine,glucose, albumin
...
layout(mat = m,heights = c(0.4,0.4,0.2))
par(mar = c(2,2,1,1))
plot(density(cohort.dt$bp, na.rm=TRUE), col='blue4', main='KDE of bloop pressure')
lines(density(cohort_complete$bp), col='red4')
plot(density(cohort.dt$diabetes, na.rm=TRUE), col='blue4', main='KDE of diabetes')
lines(density(cohort_complete$diabetes), col='red4')
...
plot(density(as.numeric(factor(cohort_complete$albumin)), na.rm=TRUE), col='blue4', main='KDE of albumin')
lines(density(as.numeric(factor(cohort_complete$albumin))), col='red4')
legend('topright', legend=c('1: normo','2: micro','3: macro'))
plot(density(cohort.dt$glucose, na.rm=TRUE), col='blue4', main='KDE of glucose')
lines(density(cohort_complete$glucose), col='red4')
plot(1, type = "n", axes=FALSE, xlab="", ylab="")
legend(x = "top",inset = 0,
        legend = c("Imputed", "Original Dataset"), 
        col=c('red4', 'blue4'), lwd=3, cex=1.5, horiz = TRUE)
```
...
layout(mat = m,heights = c(0.4,0.4,0.2))
par(mar = c(2,2,1,1))
boxplot(bp ~ diabetes, data=cohort_complete, main="Blood Pressuer stratified by Diabetes")
boxplot(urea ~ diabetes, data=cohort_complete, main="Urea stratified by Diabetes")
boxplot(creatinine ~ diabetes, data=cohort_complete, main="Creatine stratified by Diabetes")
