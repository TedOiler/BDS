...
longegfr1 = data.table(read.csv('./data/longegfr1.csv'), key=c('id', 'fu.years'))
longegfr2 = data.table(read.csv('./data/longegfr2.csv'), key=c('ID', 'fu.years'))
colnames(longegfr2) = c('id','fu.years','egfr') # change column names to make joining easier
pk = c('id','fu.years')
longegft = merge(longegfr1,
                 longegfr2,
                 by=pk,
                 all=TRUE)[order(id,
                                 fu.years)] # full outer join and sorting
longegft
```
