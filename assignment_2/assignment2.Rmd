...
univ.glm.test <- function(x, y, order=FALSE) { 
  ...
  output <- NULL
    for (i in 1:ncol(x)){
      ...
      data <- transpose(as.data.table(c(coef(summary(regr))[-1, -3])))
      data <- cbind(data, exp(coef(regr))[2])
      data <- cbind(data, colnames(X[1,])[i])
      output <- rbind(output, data)
    }
  # assign better column names 
    colnames(output) <- c("beta","std.error", "p.value", "odds ratio", "sns.name")
...
univ.glm.test(x=X,y=target, order=TRUE)
```
