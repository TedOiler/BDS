...
X.train <- data.train[, !c("diagnosis"), with=FALSE]
X.test  <- data.test [, !c("diagnosis"), with=FALSE]
...
```{r 1.c}
full.model <- lm(as.matrix(y.train) ~ as.matrix(X.train)) 
null.model <- lm(as.matrix(y.train) ~ 1)

modelB <- stepAIC(full.model, direction="back")n
