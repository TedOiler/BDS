...
roc.modelB <- roc(data.test$diagnosis, modelB.pred , plot=TRUE, col='blue4')
roc.modelS <- roc(data.test$diagnosis, modelS.pred , plot=TRUE, col='red4', add=TRUE)
roc.lasso  <- roc(data.test$diagnosis, lasso.pred  , plot=TRUE, col='orange4', add=TRUE)
roc.ridge  <- roc(data.test$diagnosis, ridge.pred  , plot=TRUE, col='green4', add=TRUE)
...
for (fold in 1:length(folds)) {
  gdm.dt.train <- gdm.dt[-folds[[fold]]]
  crude <- NULL
  for (snp in 3:ncol(gdm.dt.train)) { # skip the outcome column;
    crude <- rbind(crude,
    univ.glm.test(gdm.dt.train[[snp]], gdm.dt.train[[2]]))
    }
  crude <- cbind(snp=colnames(gdm.dt)[-1], crude)
  crude.folds[[fold]] <- crude
}
```
...
cor.nki <- cor.nki[-which(rowSums(cor.nki)==1),-which(colSums(cor.nki)==1)]
corrplot(cor.nki,
         order="hclust",
         addrect=2,
         method="square",
